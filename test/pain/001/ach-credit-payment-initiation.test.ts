...
import type { ABAAgent, BaseAccount } from '../../../src/lib/types';
import {
  ACHCreditPaymentInitiation,
  type ACHCreditPaymentInitiationConfig,
} from '../../../src/pain/001/ach-credit-payment-initiation';
...
describe('ACHCreditPaymentInitiation', () => {
  ...
  describe('serialize', () => {
    ...
    test('should serialize to XML', () => {
      ...
      expect(xml).toMatch(
        /<LclInstrm>\s*<Prtry>\s*CCD\s*<\/Prtry>\s*<\/LclInstrm>/,
      );
    });
  ...
  describe('fromXML', () => {
    describe('with a goldman sachs ACH 001 XML file', () => {
      ...
      test('should parse into an ACHCreditPaymentInitiation', () => {
        expect(achPayment).toBeInstanceOf(ACHCreditPaymentInitiation);
        expect(achPayment.messageId).toBe('Message-Id');
        expect(achPayment.creationDate).toStrictEqual(
          new Date('2024-05-10T16:10:02.017+00:00'),
        );
      });
    });
    describe('with a cross river ACH 001 XML file', () => {
      ...
      test('should parse into an ACHCreditPaymentInitiation', () => {
        ...
        const instruction = achPayment.paymentInstructions[0]!;
        expect(instruction.id).toBe('234ACHC123455');
        expect(instruction.amount).toBe(1);
        expect(instruction.currency).toBe('USD');
        expect(instruction.creditor.name).toBe('John Doe Funding LLC');
        expect(
          (instruction.creditor?.account as BaseAccount)?.accountNumber,
        ).toBe('0123456789');
        expect((instruction.creditor.agent as ABAAgent)?.abaRoutingNumber).toBe(
          '123456789',
        );
    ...
    describe('with generated ACH 001 XML file', () => {
      ...
      const recreatedAchPayment =
        ACHCreditPaymentInitiation.fromXML(serializedXml);
      ...
      test('information should be correctly parsed', () => {
        ...
        expect(recreatedAchPayment.initiatingParty.name).toBe(
        ...
        expect(parsedInstruction1.endToEndId).toBe(
          paymentInstruction1.endToEndId,
        );
        ...
        expect(parsedInstruction1.creditor.name).toBe(
          paymentInstruction1.creditor.name,
        );
        expect(parsedInstruction1.remittanceInformation).toBe(
          paymentInstruction1.remittanceInformation,
        );
        ...
        expect(parsedInstruction2.endToEndId).toBe(
          paymentInstruction2.endToEndId,
        );
        ...
        expect(() => {
          new ACHCreditPaymentInitiation({
            ...
            paymentInstructions: [
              {
                ...paymentInstruction1,
                // @ts-expect-error - Intentionally using wrong currency to test validation
                currency: 'EUR',
              },
            ],
          });
